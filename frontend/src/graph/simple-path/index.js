 ...
 */
export function allSimplePaths(graph, source, target, options) {
  if (!graph.hasNode(source))
    throw new Error(
      'graphology-simple-path.allSimplePaths: expecting: could not find source node "' + source + '" in the graph.'
    )

  if (!graph.hasNode(target))
    throw new Error(
      'graphology-simple-path.allSimplePaths: expecting: could not find target node "' + target + '" in the graph.'
    )
  ...
  while (stack.length !== 0) {
    ...
    if (!child) {
      ...
      if (!visited.has(target) && stack.length < maxDepth) stack.push(graph.outboundNeighbors(child))
      else visited.pop()
    }
  }
...
function collectEdges(graph, source) {
  ...
  graph.forEachOutboundEdge(source, function (edge, attr, ext1, ext2) {
    if (window.state.lowcostOnly && !attr.lowcost) return
 ...
 * @return {array}              - The found paths.
 */
export function allSimpleEdgePaths(graph, source, target, options) {
  if (graph.multi) throw new Error('graphology-simple-path.allSimpleEdgePaths: not implemented for multi graphs.')

  if (!graph.hasNode(source))
    throw new Error(
      'graphology-simple-path.allSimpleEdgePaths: expecting: could not find source node "' + source + '" in the graph.'
    )

  if (!graph.hasNode(target))
    throw new Error(
      'graphology-simple-path.allSimpleEdgePaths: expecting: could not find target node "' + target + '" in the graph.'
    )
  ...
  while (stack.length !== 0) {
    ...
    if (!record) {
      ...
      if (!visited.has(target) && stack.length < maxDepth) stack.push(collectEdges(graph, child))
      else visited.pop()
    }
  }
 ...
 * @return {array}              - The found paths.
 */
export function allSimpleEdgeGroupPaths(graph, source, target, options) {
  if (!graph.hasNode(source))
    throw new Error(
  ...
  while (stack.length !== 0) {
    ...
    if (!record) {
      ...
      if (!visited.has(target) && stack.length < maxDepth) stack.push(collectMultiEdges(graph, child))
      else visited.pop()
    }
  }
